...
Historical source peaks and throughput are unavailable, so the source table shows only current workload and the API returns `null` source peaks. With `--json`, the command preserves the API response shape, including those `null` values, routing percentage, refresh time, and both observation timestamps. During a mixed backend rollout or rollback, `source` may be absent; in that case the human output remains the previous destination-only view. An API response may also omit `next_refresh_at`; human-readable output then omits the refresh line.
